...
model = load_model('Stockpriceflask.h5')
scaler = MinMaxScaler()
...
def update_data():
    while True:
        last_price = df['Close'].iloc[-10:]  # Get the last stock price

        sequence_length = 10  # Make sure it matches the training sequence length
